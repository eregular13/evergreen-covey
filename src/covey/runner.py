...
def _install_httpx_release() -> Path:
    """Download the official httpx release onto this VM. Never into git."""
    machine = platform.machine().lower()
    if machine not in {"x86_64", "amd64"}:
        raise RunnerError(
            f"no httpx GitHub release mapping for machine={machine!r}; "
            "install httpx yourself and set COVEY_HTTPX"
        )
    dest_dir = Path.home() / ".local" / "bin"
    dest_dir.mkdir(parents=True, exist_ok=True)
    dest = dest_dir / "httpx"
    try:
        with tempfile.TemporaryDirectory(prefix="covey-httpx-") as tmp:
            work = Path(tmp)
            archive = work / "httpx.zip"
            urllib.request.urlretrieve(HTTPX_LINUX_X64_URL, archive)
            with zipfile.ZipFile(archive) as zf:
                zf.extractall(work)
            found = None
            for path in work.rglob("httpx"):
                if path.is_file() and os.access(path, os.X_OK):
                    found = path
                    break
            if found is None:
                raise RunnerError("httpx binary missing from extracted release")
            dest.write_bytes(found.read_bytes())
            dest.chmod(0o755)
    except urllib.error.URLError as exc:
        raise RunnerError(f"download httpx {HTTPX_RELEASE} failed: {exc}") from exc
    except RunnerError:
        raise
    except Exception as exc:
        raise RunnerError(f"unpack httpx {HTTPX_RELEASE} failed: {exc}") from exc
    if not dest.is_file():
        raise RunnerError("httpx download finished but binary is missing")
    _prepend_path(dest_dir)
    return dest


def _go_install_httpx() -> Path:
    go = shutil.which("go")
    if go is None:
        raise RunnerError("go not on PATH; cannot go-install httpx")
    dest_dir = Path.home() / ".local" / "bin"
    dest_dir.mkdir(parents=True, exist_ok=True)
    env = os.environ.copy()
    env["GOBIN"] = str(dest_dir)
    completed = subprocess.run(
        [go, "install", f"github.com/projectdiscovery/httpx/cmd/httpx@v{HTTPX_RELEASE}"],
        check=False,
        capture_output=True,
        text=True,
        env=env,
    ...
    if completed.returncode != 0:
        raise RunnerError(
            "go install httpx failed: " + (completed.stderr or completed.stdout)[-400:]
        )
    dest = dest_dir / "httpx"
    if not dest.is_file():
        raise RunnerError("go install httpx finished but binary is missing")
    _prepend_path(dest_dir)
    return dest


def ensure_sslscan(*, install_if_missing: bool = False) -> ExecSpec:
    """Resolve BYO sslscan. Optionally apt-install onto this VM only."""
    try:
        return resolve_exec("sslscan")
    except RunnerError:
        if not install_if_missing:
            raise
    if shutil.which("apt-get") is None:
    ...
    if install.returncode != 0:
        raise RunnerError(f"apt-get install sslscan failed: {install.stderr[-400:]}")
    found = shutil.which("sslscan")
    if not found:
...
def ensure_whatweb(*, install_if_missing: bool = False) -> ExecSpec:
    """Resolve BYO whatweb. Optionally apt-install onto this VM only."""
    try:
        return resolve_exec("whatweb")
    except RunnerError:
        if not install_if_missing:
            raise
    if shutil.which("apt-get") is None:
    ...
    if install.returncode != 0:
        raise RunnerError(f"apt-get install whatweb failed: {install.stderr[-400:]}")
    found = shutil.which("whatweb")
    if not found:
        raise RunnerError("whatweb installed but still not on PATH")
    return ExecSpec(kind="local", binary=found, display=found, entrypoint="whatweb")


def ensure_onesixtyone(*, install_if_missing: bool = False) -> ExecSpec:
    """Resolve BYO onesixtyone. Optionally apt-install onto this VM only."""
    try:
        return resolve_exec("onesixtyone")
    except RunnerError:
        if not install_if_missing:
            raise
    if shutil.which("apt-get") is None:
    ...
    if install.returncode != 0:
        raise RunnerError(
            f"apt-get install onesixtyone failed: {install.stderr[-400:]}"
        )
    found = shutil.which("onesixtyone")
    if not found:
        raise RunnerError("onesixtyone installed but still not on PATH")
    return ExecSpec(
        kind="local",
        binary=found,
        display=found,
        entrypoint="onesixtyone",
    )
...
def ensure_braa(*, install_if_missing: bool = False) -> ExecSpec:
    """Resolve BYO braa. Optionally apt-install onto this VM only."""
    try:
        return resolve_exec("braa")
    except RunnerError:
        if not install_if_missing:
            raise
    if shutil.which("apt-get") is None:
    ...
    if install.returncode != 0:
        raise RunnerError(f"apt-get install braa failed: {install.stderr[-400:]}")
    found = shutil.which("braa")
    if not found:
        raise RunnerError("braa installed but still not on PATH")
    return ExecSpec(kind="local", binary=found, display=found, entrypoint="braa")


def ensure_ike_scan(*, install_if_missing: bool = False) -> ExecSpec:
    """Resolve BYO ike-scan. Optionally apt-install onto this VM only."""
    try:
        return resolve_exec("ike-scan")
    except RunnerError:
        if not install_if_missing:
            raise
    if shutil.which("apt-get") is None:
    ...
    if install.returncode != 0:
        raise RunnerError(f"apt-get install ike-scan failed: {install.stderr[-400:]}")
    found = shutil.which("ike-scan")
    if not found:
        raise RunnerError("ike-scan installed but still not on PATH")
...
def ensure_httpx(*, install_if_missing: bool = False) -> ExecSpec:
    """Resolve BYO httpx. Optionally fetch a release onto this VM only."""
    try:
        return resolve_exec("httpx")
    except RunnerError as missing:
        if not install_if_missing:
            raise
        last = missing
    ...
    try:
        ...
        errors.append(str(exc))
    raise RunnerError(
        "httpx not available and prove-install failed (binary stays off git). "
        + " | ".join(errors)
    )
