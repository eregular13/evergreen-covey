...
| Covey surface | Signed SCOPE ranges | pass1 hosts + pass2 open ports + http/tls misconfigs when sampled | *Not this lane* | `covey prove` / `covey export --target all` | asset: host, service · finding: open_port_observed, misconfig_observed · evidence: scan.xml · poam: mapped misconfigs only | Open port ≠ vuln ≠ control failure. Unknown stays UNMAPPED. |
